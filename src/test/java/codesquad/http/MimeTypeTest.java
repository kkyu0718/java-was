...
class MimeTypeTest {
    ...
    void MimeType_Enum이_주어지고_css파일을_변환하면_MimeType_CSS로_변환된다() {
        MimeType mimeType = MimeType.fromExt(Path.of("index.css"));
    ...
    void MimeType_Enum이_주어지고_js파일을_변환하면_MimeType_JS로_변환된다() {
        MimeType mimeType = MimeType.fromExt(Path.of("index.js"));
    ...
    void MimeType_Enum이_주어지고_html파일을_변환하면_MimeType_HTML로_변환된다() {
        MimeType mimeType = MimeType.fromExt(Path.of("index.html"));
    ...
    void MimeType_Enum이_주어지고_정의되지않은_확장자_파일을_변환하면_Illegal_Argument에러가_발생한다() {
        Assertions.assertThrows(IllegalArgumentException.class, () -> MimeType.fromExt(Path.of("index.abcd")));
    }
}